...
const changePassword = async (currentPassword, newPassword, regno) => {
  ...
  try {
    const res = await axios({
      method: "PATCH",
      url: "/api/v1/common/passwordUpdate",
      data: {
        currentPassword,
        newPassword,
        regno,
...
btnPassword.addEventListener("click", (e) => {
  ...
  if (currentPassword.length < 1 || newPassword < 1) {
    return console.log("you must eneter both current and new passwords");
  }
...
const messageSend = async (message, regno, time, flag) => {
  if (navigator.onLine === false) {
    return console.log("you are not connected to the innternet");
  }
  try {
    const res = await axios({
      method: "post",
      url: "http://127.0.0.1:8000//api/v1/common/messageSend",
      data: {
        message,
        regno,
        time,
...
messageBtn.addEventListener("click", (e) => {
  ...
  let flag = "sender";
  if (message.length < 1) {
    return console.log("you cannot sent an empty message");
  }
